...
class SpriteTemplate {
  ...
  SpriteTemplate({@required this.pegBoard, @required this.spriteTemplateElements});

  static SpriteTemplate fromMap(PegBoard pegBoard, int width, int height, List<int> templateMap) {
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        if (templateMap[dataOffset] != -1) {
          templateElements.add(SpriteTemplateElement(Coord(x, y), templateMap[dataOffset]));
        }
    ...
    return SpriteTemplate(pegBoard: pegBoard, spriteTemplateElements: templateElements);
  }
  ...
  Sprite renderSprite(List<Color> colors) {
    ...
    spriteTemplateElements.forEach((spriteTemplateElement) {
      final color = spriteTemplateElement.colorIndex < colors.length ? colors[spriteTemplateElement.colorIndex] : Colors.white;
